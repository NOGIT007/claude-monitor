...
// Scheduled DB maintenance — runs hourly
function runDbMaintenance() {
  try {
    ...
    // Sessions must be deleted after token_usage due to FK constraint
    db.run("DELETE FROM sessions WHERE last_seen_at < datetime('now', '-90 days')");
    db.run("PRAGMA wal_checkpoint(TRUNCATE)");
    console.log("[maintenance] DB maintenance completed");
  } catch (err) {
    console.error("[maintenance] DB maintenance failed:", err);
  }
}

const dbMaintenanceTimer = setInterval(runDbMaintenance, 60 * 60 * 1000);
